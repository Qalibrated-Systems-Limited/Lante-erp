...
export default function FieldVehicleDetailPage() {
  ...
  useEffect(() => {
    setLoading(true)
    api.get(`/api/v1/field-vehicles/${id}`)
  ...
  useEffect(() => {
    ...
    api.get(`/api/v1/attachments?entityType=FieldVehicle&entityId=${id}`)
  ...
  useEffect(() => {
    ...
    api.get(`/api/v1/field-vehicles/${id}/dispatches`)
    ...
    return (
      <>
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-gray-400">
          ...
          <button
            onClick={() => navigate('/modules/operations/field-vehicles')}
  ...
  const serviceSoon = nextServiceDate && !serviceOverdue && (nextServiceDate - today) < 30 * 24 * 60 * 60 * 1000
  // `new Date(vehicle.insuranceExpiry) < today` fails open on any unparseable value (#375) —
  // e.g. "31/01/2020" or "expired" both silently read as "not expired". expiryState reports
  // that as its own 'unknown' state instead, matching the fleet/ copy of this page (#376).
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
          <div className="flex items-start gap-4">
            <div className="w-16 h-16 rounded-2xl bg-amber-50 border border-amber-100 flex items-center justify-center text-3xl flex-shrink-0">
              🚗
            </div>
            <div>
          ...
          {(serviceOverdue || serviceSoon || insuranceExpired || insuranceUnreadable) && (
            <div className="flex flex-col gap-1.5">
              ...
              {insuranceUnreadable && (
                <span className="inline-flex items-center gap-1.5 text-xs font-medium bg-amber-50 text-amber-700 border border-amber-100 rounded-lg px-2.5 py-1">
                  ⚠ Insurance expiry unreadable — check the record
                </span>
              )}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-5">
              ...
              <dl className="space-y-3">
                ...
                <Row
                  label="Next Service"
                  value={fmt(vehicle.nextServiceDate)}
          ...
          <div>
            {photosLoading ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                {photos.map(p => (
                  <a key={p.id} href={`${API_URL}${p.url}`} target="_blank" rel="noopener noreferrer">
                    <img
                      src={`${API_URL}${p.url}`}
