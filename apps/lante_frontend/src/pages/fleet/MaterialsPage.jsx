...
import { exportToPdf, exportToExcel, MATERIAL_COLUMNS } from '../../utils/export.js'
...
export default function MaterialsPage() {
  ...
  const [modal, setModal]     = useState(null)   // null | 'create' | { ...material }
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const res = await api.get('/api/v1/materials', { params: { pageNumber: page, pageSize: PAGE_SIZE, search: search || undefined } })
  ...
  async function fetchExportRows() {
    const res = await api.get('/api/v1/materials', { params: { pageNumber: 1, pageSize: EXPORT_CAP, search: search || undefined } })
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      await exportToPdf({
        ...
        rows,
        filename: `Materials-${new Date().toISOString().slice(0,10)}`,
        theme: 'navy',
        docModule: 'FLEET',
      })
    } catch {
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      if (modal === 'create') {
        ...
        await api.put(`/api/v1/materials/${modal.id}`, payload)
      }
      setModal(null)
      load()
    } catch (err) {
      setFormErr(err.response?.data?.message ?? 'Failed to save material.')
    } finally {
      setSaving(false)
    }
  }
  ...
  async function confirmDelete() {
    ...
    try {
      await api.delete(`/api/v1/materials/${materialToDelete.id}`)
      load()
    } catch (err) {
  ...
  return (
      <>
        <main className="w-full px-4 sm:px-6 py-8">
          ...
          <Collapsible title="About Materials" dismissKey="fleet.pageInfo.materials.dismissed">
            <p className="text-sm text-gray-700">
              The materials list used when logging what a loaded trip is carrying.
            </p>
          </Collapsible>
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-extrabold text-navy">Materials</h1>
              <p className="text-sm text-gray-500 mt-0.5">
                {loading ? 'Loading…' : `${totalCount} material${totalCount !== 1 ? 's' : ''}`}
              </p>
            </div>
          ...
          <div className="relative mb-6">
            ...
            <input
                ...
                className="w-full pl-11 pr-10 py-3 bg-gray-100 rounded-xl text-sm border-0 focus:outline-none focus:ring-2 focus:ring-gold"
            />
            {search && (
                <button
          ...
          {loading ? (
              <div className="space-y-3">
                {[1,2,3,4].map(i => <div key={i} className="h-16 bg-white rounded-2xl border animate-pulse" />)}
              </div>
          ) : materials.length === 0 ? (
              ...
              <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tr className="bg-gray-50 border-b border-gray-100">
                    <th className="text-left px-6 py-4 text-xs font-semibold text-gray-600 uppercase tracking-wider">Material Name</th>
            ...
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
              <form onSubmit={handleSave} className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md space-y-4">
                <h3 className="font-bold text-gray-900 text-lg">
                  {modal === 'create' ? 'New Material' : `Edit — ${modal.name}`}
                </h3>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  >
                    {saving ? 'Saving…' : modal === 'create' ? 'Create Material' : 'Save Changes'}
                  </button>
                  <button
                      type="button"
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-gray-900">Delete Material</h3>
                ...
                <p className="text-sm text-gray-600">
                  ...
                  <span className="font-semibold text-gray-900">"{materialToDelete.name}"</span>?
                </p>
