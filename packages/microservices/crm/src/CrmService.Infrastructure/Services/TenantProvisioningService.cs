...
/// <summary>Creates and migrates the crm tables into a tenant's Postgres schema.</summary>
...
public partial class TenantProvisioningService : ITenantProvisioningService
{
    ...
    public async Task<ProvisioningResult> ProvisionAsync(string schema, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var lockKey = $"crm-provision:{schema}";
            ...
            try
            {
#pragma warning disable EF1002 // schema name is validated by IsSafeSchema above (not user input); identifiers can't be parameterized via ExecuteSqlAsync anyway.
...
#pragma warning restore EF1002
            ...
            _logger.LogInformation("Provisioned crm schema {Schema}", schema);
            return new ProvisioningResult(true, schema, null);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed provisioning crm schema {Schema}", schema);
    ...
    private TenantCrmDbContext BuildTenantContext(string schema)
    {
        ...
        var options = new DbContextOptionsBuilder<TenantCrmDbContext>()
            ...
            {
                o.MigrationsAssembly("CrmService.Infrastructure");
    ...
    private async Task<string?> GrantAppRoleAsync(TenantCrmDbContext ctx, string schema, CancellationToken ct)
    {
        var appRole = _configuration["ProvisioningAppRole"] ?? "qalicore_app";
        if (!SafeSchemaRegex().IsMatch(appRole)) return null;
        try
        {
#pragma warning disable EF1002 // appRole is validated by SafeSchemaRegex above (not user input); identifiers can't be parameterized via ExecuteSqlAsync/SqlQueryRaw anyway.
...
#pragma warning disable EF1002 // schema/appRole are validated by IsSafeSchema/SafeSchemaRegex above (not user input); identifiers can't be parameterized via ExecuteSqlAsync anyway.
