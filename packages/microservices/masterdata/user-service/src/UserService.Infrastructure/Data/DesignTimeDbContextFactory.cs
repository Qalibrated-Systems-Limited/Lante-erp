...
/// </summary>
public class DesignTimeDbContextFactory : IDesignTimeDbContextFactory<LanteUserServiceDbContext>
{
    private const string ApiProject = "UserService.Api";

    public LanteUserServiceDbContext CreateDbContext(string[] args)
    {
        ...
        var optionsBuilder = new DbContextOptionsBuilder<LanteUserServiceDbContext>();
        optionsBuilder.UseNpgsql(connectionString, o => o.MigrationsAssembly("UserService.Infrastructure"));

        return new LanteUserServiceDbContext(optionsBuilder.Options);
    }
}
