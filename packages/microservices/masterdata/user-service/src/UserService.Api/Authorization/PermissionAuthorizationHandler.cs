...
public class PermissionAuthorizationHandler : AuthorizationHandler<PermissionRequirement>
{
    private static readonly Dictionary<string, string[]> _hierarchy = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["users.read"]         = ["users.read", "users.write", "users.delete", "system.admin",
                                  "operations.write", "operations.approve",
                                  "projects.write", "projects.approve"],
        ["users.write"]        = ["users.write", "users.delete", "system.admin"],
        ["users.delete"]       = ["users.delete", "system.admin"],
    ...
    {
        if (context.User?.Identity?.IsAuthenticated != true || string.IsNullOrWhiteSpace(requirement.Permission))
        {
            context.Fail();
