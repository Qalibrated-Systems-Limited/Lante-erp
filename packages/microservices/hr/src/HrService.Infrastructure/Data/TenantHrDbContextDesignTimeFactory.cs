...
/// <summary>
/// Design-time factory for <see cref="TenantHrDbContext"/>. Emits schema-agnostic migrations
/// that apply into whichever tenant schema the connection's search_path points at.
/// </summary>
public class TenantHrDbContextDesignTimeFactory : IDesignTimeDbContextFactory<TenantHrDbContext>
{
    private const string ApiProject = "HrService.Api";

    public TenantHrDbContext CreateDbContext(string[] args)
    {
        ...
        var optionsBuilder = new DbContextOptionsBuilder<TenantHrDbContext>();
        optionsBuilder.UseNpgsql(connectionString, o => o.MigrationsAssembly("HrService.Infrastructure"));

        return new TenantHrDbContext(optionsBuilder.Options);
    }
}
