...
/// <summary>Creates and migrates the hr tables into a tenant's Postgres schema.</summary>
...
public partial class TenantProvisioningService : ITenantProvisioningService
{
    ...
    public async Task<ProvisioningResult> ProvisionAsync(string schema, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var lockKey = $"hr-provision:{schema}";
...
#pragma warning restore EF1002
            ...
            _logger.LogInformation("Provisioned hr schema {Schema}", schema);
            return new ProvisioningResult(true, schema, null);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed provisioning hr schema {Schema}", schema);
    ...
    private TenantHrDbContext BuildTenantContext(string schema)
    {
        ...
        var options = new DbContextOptionsBuilder<TenantHrDbContext>()
            .UseNpgsql(connectionString, o =>
            {
                o.MigrationsAssembly("HrService.Infrastructure");
        ...
        return new TenantHrDbContext(options);
    }
    ...
    private async Task<string?> GrantAppRoleAsync(TenantHrDbContext ctx, string schema, CancellationToken ct)
    {
...
#pragma warning disable EF1002 // schema and appRole are both validated by IsSafeSchema/SafeSchemaRegex above — not user input, and identifiers can't be parameterized via ExecuteSqlAsync anyway.
