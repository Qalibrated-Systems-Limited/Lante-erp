...
using HrService.Api.Middleware;
using HrService.Core.Mappings;
using HrService.Infrastructure.Data;
using HrService.Infrastructure.ServiceRegistration;
...
try
{
    ...
    Log.Information("Starting QaliCore HrService...");
    ...
    services.AddOpenTelemetry()
        .ConfigureResource(r => r.AddService(serviceName: "hr-service"))
        .WithTracing(t => t
    ...
    services.AddSingleton<HrService.Api.Services.PayrollPdfService>();   // H6 — payslip + P9 rendering
    services.AddScoped<HrService.Infrastructure.Services.ITenantProvisioningService,
                       HrService.Infrastructure.Services.TenantProvisioningService>();
    ...
    var jwksResolver = new HrService.Api.Authentication.JwtIssuerSigningKeyResolver(new HttpClient(), jwksUrl);
    ...
    services.AddSwaggerGen(c =>
    {
        ...
        {
            Title = "QaliCore HR & Payroll API",
            Version = "v1",
            Description = "QaliCore — Human Resources & Payroll (Module 3)"
        });
    ...
    if (app.Environment.IsDevelopment())
    {
        app.UseSwaggerUI(c =>
        {
            c.SwaggerEndpoint("/swagger/v1/swagger.json", "QaliCore HrService API V1");
    ...
    else
    {
        app.UseSwaggerUI(c =>
        {
            c.SwaggerEndpoint("v1/swagger.json", "QaliCore HrService API V1");
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        var migrationOptions = new DbContextOptionsBuilder<HrDbContext>()
            .UseNpgsql(migrationConn, o => o.CommandTimeout(60))
            .Options;

        await using var migrationDb = new HrDbContext(migrationOptions);
        ...
        var provisioning = scope.ServiceProvider
            .GetRequiredService<HrService.Infrastructure.Services.ITenantProvisioningService>();
    ...
    Log.Information("QaliCore HrService started successfully.");
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "QaliCore HrService terminated unexpectedly.");
    throw;
}
finally
{
