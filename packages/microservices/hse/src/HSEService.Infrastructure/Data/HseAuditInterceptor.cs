...
/// <summary>
/// Records every HSE write into <see cref="HseAuditLog"/> — same shape as finance's
...
/// </summary>
public class HseAuditInterceptor(IHttpContextAccessor httpContextAccessor) : SaveChangesInterceptor
{
    ...
    private void Record(DbContext? db)
    {
        ...
        var entries = db.ChangeTracker.Entries()
            .Where(e => e.Entity is not HseAuditLog)
            .Where(e => e.State is EntityState.Added or EntityState.Modified or EntityState.Deleted)
            .ToList();

        foreach (var entry in entries)
        {
            db.Add(new HseAuditLog
            {
