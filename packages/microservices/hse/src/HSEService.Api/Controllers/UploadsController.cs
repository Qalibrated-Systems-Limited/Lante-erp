...
// Shared file upload for records that carry a document/certificate URL (RAMS, training
// certificates, statutory inspection certificates). One endpoint + one PVC instead of a
// separate upload path per entity.
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/hse-uploads")]
[Authorize(Policy = "hse.write")]
public class UploadsController(IConfiguration config) : ControllerBase
{
    private static readonly HashSet<string> AllowedFolders = ["rams", "training", "inspections"];
    ...
    public async Task<ActionResult<ApiResponse<object>>> Upload([FromForm] string folder, IFormFile file)
    {
        ...
        var url = $"{publicBaseUrl}/uploads/hse/{folder}/{fileName}";
