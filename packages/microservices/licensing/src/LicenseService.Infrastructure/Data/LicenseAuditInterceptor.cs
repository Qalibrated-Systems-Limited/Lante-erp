...
/// <summary>
/// Records every licensing write into <see cref="LicenseAuditLog"/> — same shape as finance's
...
/// </summary>
public class LicenseAuditInterceptor(IHttpContextAccessor httpContextAccessor) : SaveChangesInterceptor
{
    ...
    private void Record(DbContext? db)
    {
        ...
        var entries = db.ChangeTracker.Entries()
            .Where(e => e.Entity is not LicenseAuditLog)
            .Where(e => e.State is EntityState.Added or EntityState.Modified or EntityState.Deleted)
            .ToList();

        foreach (var entry in entries)
        {
            db.Add(new LicenseAuditLog
            {
