...
/// <summary>Creates and migrates the licensing tables into a tenant's Postgres schema.</summary>
...
#pragma warning disable EF1002 // schema name is validated by IsSafeSchema above (line 33) before use — not user input, and identifiers can't be parameterized via ExecuteSqlAsync anyway.
...
#pragma warning restore EF1002
            ...
            _logger.LogInformation("Provisioned licensing schema {Schema}", schema);
            return new ProvisioningResult(true, schema, null);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed provisioning licensing schema {Schema}", schema);
            return new ProvisioningResult(false, schema, ex.Message);
        }
    }

    private TenantLicenseDbContext BuildTenantContext(string schema)
    {
        ...
        var options = new DbContextOptionsBuilder<TenantLicenseDbContext>()
            .UseNpgsql(connectionString, o =>
            {
                o.MigrationsAssembly("LicenseService.Infrastructure");
                o.MigrationsHistoryTable("__EFMigrationsHistory", schema);
            })
            .ConfigureWarnings(w => w.Ignore(RelationalEventId.PendingModelChangesWarning))
            .Options;

        return new TenantLicenseDbContext(options);
    }
    ...
    private async Task<string?> GrantAppRoleAsync(TenantLicenseDbContext ctx, string schema, CancellationToken ct)
    {
        var appRole = _configuration["ProvisioningAppRole"] ?? "qalicore_app";
        if (!SafeSchemaRegex().IsMatch(appRole)) return null;
        try
        {
#pragma warning disable EF1002 // appRole is validated by SafeSchemaRegex above (line 83) before use — not user input, and identifiers can't be parameterized via ExecuteSqlAsync anyway.
...
#pragma warning disable EF1002 // schema and appRole are validated by IsSafeSchema/SafeSchemaRegex above (lines 33, 83) before use — not user input, and identifiers can't be parameterized via ExecuteSqlAsync anyway.
