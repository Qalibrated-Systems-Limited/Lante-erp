...
/// <summary>
/// Records every compliance write into <see cref="ComplianceAuditLog"/> — same shape as finance's
...
/// </summary>
public class ComplianceAuditInterceptor(IHttpContextAccessor httpContextAccessor) : SaveChangesInterceptor
{
    ...
    private void Record(DbContext? db)
    {
        ...
        var entries = db.ChangeTracker.Entries()
            .Where(e => e.Entity is not ComplianceAuditLog)
            .Where(e => e.State is EntityState.Added or EntityState.Modified or EntityState.Deleted)
            .ToList();

        foreach (var entry in entries)
        {
            db.Add(new ComplianceAuditLog
            {
