...
/// <summary>Creates and migrates the procurement tables into a tenant's Postgres schema.</summary>
...
#pragma warning disable EF1002 // schema is validated by IsSafeSchema/SafeSchemaRegex above — not user input, and identifiers can't be parameterized via ExecuteSqlAsync anyway.
...
#pragma warning restore EF1002
            ...
            _logger.LogInformation("Provisioned procurement schema {Schema}", schema);
            return new ProvisioningResult(true, schema, null);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed provisioning procurement schema {Schema}", schema);
            return new ProvisioningResult(false, schema, ex.Message);
        }
    }

    private TenantProcurementDbContext BuildTenantContext(string schema)
    {
        ...
        var options = new DbContextOptionsBuilder<TenantProcurementDbContext>()
            .UseNpgsql(connectionString, o =>
            {
                o.MigrationsAssembly("ProcurementService.Infrastructure");
        ...
        return new TenantProcurementDbContext(options);
    }
    ...
    private async Task<string?> GrantAppRoleAsync(TenantProcurementDbContext ctx, string schema, CancellationToken ct)
    {
        var appRole = _configuration["ProvisioningAppRole"] ?? "qalicore_app";
        if (!SafeSchemaRegex().IsMatch(appRole)) return null;
        try
        {
#pragma warning disable EF1002 // appRole is validated by SafeSchemaRegex above — not user input, and identifiers can't be parameterized via ExecuteSqlAsync anyway.
...
#pragma warning disable EF1002 // schema is validated by IsSafeSchema and appRole by SafeSchemaRegex before this method is reached — neither is user input, and identifiers can't be parameterized via ExecuteSqlAsync anyway.
