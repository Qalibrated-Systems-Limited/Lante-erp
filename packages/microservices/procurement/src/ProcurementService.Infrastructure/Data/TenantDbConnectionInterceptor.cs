...
/// user identity). RLS is dropped; this only ever sets <c>search_path</c> — see #227.
/// </summary>
public partial class TenantDbConnectionInterceptor : DbConnectionInterceptor
{
    ...
    // retain prior session state, so skipping on a no-claim request would leak the previous tenant.
