...
using ProcurementService.Api.Middleware;
using ProcurementService.Core.Mappings;
using ProcurementService.Infrastructure.Data;
using ProcurementService.Infrastructure.ServiceRegistration;
...
try
{
    ...
    Log.Information("Starting QaliCore ProcurementService...");
    ...
    services.AddOpenTelemetry()
        .ConfigureResource(r => r.AddService(serviceName: "procurement-service"))
        .WithTracing(t => t
    ...
    services.AddScoped<ProcurementService.Infrastructure.Services.ITenantProvisioningService,
                       ProcurementService.Infrastructure.Services.TenantProvisioningService>();
    ...
    var jwksResolver = new ProcurementService.Api.Authentication.JwtIssuerSigningKeyResolver(new HttpClient(), jwksUrl);
    ...
    services.AddSwaggerGen(c =>
    {
        ...
        {
            Title = "QaliCore ProcurementService API",
            Version = "v1",
            Description = "QaliCore — Procurement & Supply Chain (Module 4)"
        });
    ...
    if (app.Environment.IsDevelopment())
    {
        app.UseSwaggerUI(c =>
        {
            c.SwaggerEndpoint("/swagger/v1/swagger.json", "QaliCore ProcurementService API V1");
    ...
    else
    {
        app.UseSwaggerUI(c =>
        {
            c.SwaggerEndpoint("v1/swagger.json", "QaliCore ProcurementService API V1");
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        var migrationOptions = new DbContextOptionsBuilder<ProcurementDbContext>()
            .UseNpgsql(migrationConn, o => o.CommandTimeout(60))
            .Options;

        await using var migrationDb = new ProcurementDbContext(migrationOptions);
        ...
        // behind (missing columns/tables) until someone notices. ProvisionAsync's migrate/grant
        ...
        var provisioning = scope.ServiceProvider
            .GetRequiredService<ProcurementService.Infrastructure.Services.ITenantProvisioningService>();
    ...
    Log.Information("QaliCore ProcurementService started successfully.");
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "QaliCore ProcurementService terminated unexpectedly.");
    throw;
}
finally
{
