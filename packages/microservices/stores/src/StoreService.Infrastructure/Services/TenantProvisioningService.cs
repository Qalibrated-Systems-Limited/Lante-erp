...
/// <summary>Creates and migrates the stores tables into a tenant's Postgres schema.</summary>
...
#pragma warning restore EF1002
            ...
            _logger.LogInformation("Provisioned stores schema {Schema}", schema);
            return new ProvisioningResult(true, schema, null);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed provisioning stores schema {Schema}", schema);
            return new ProvisioningResult(false, schema, ex.Message);
        }
    }

    private TenantStoreDbContext BuildTenantContext(string schema)
    {
        ...
        var options = new DbContextOptionsBuilder<TenantStoreDbContext>()
            ...
            {
                o.MigrationsAssembly("StoreService.Infrastructure");
    ...
    private async Task<string?> GrantAppRoleAsync(TenantStoreDbContext ctx, string schema, CancellationToken ct)
    {
        var appRole = _configuration["ProvisioningAppRole"] ?? "qalicore_app";
        if (!SafeSchemaRegex().IsMatch(appRole)) return null;
        try
        {
#pragma warning disable EF1002 // role name is validated by SafeSchemaRegex above before use — not user input, and identifiers can't be parameterized via ExecuteSqlAsync anyway.
...
#pragma warning disable EF1002 // schema/role names are validated by IsSafeSchema/SafeSchemaRegex above before use — not user input, and identifiers can't be parameterized via ExecuteSqlAsync anyway.
