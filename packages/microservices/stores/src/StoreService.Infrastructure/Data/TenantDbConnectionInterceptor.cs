...
/// <summary>Binds each opened connection to the caller's tenant schema (Postgres search_path).
/// JWT `schema` claim wins over the `X-Tenant-Schema` header — the header is only a fallback for
/// callers with no tenant-scoped JWT claim (internal/service-key calls). ALWAYS sets search_path,
/// even with no claim, since pooled connections retain the previous caller's setting.</summary>
public partial class TenantDbConnectionInterceptor : DbConnectionInterceptor
{
    ...
    private string? Resolve()
    {
        ...
        // anonymous end users hitting [AllowAnonymous] routes can set arbitrary headers on their
        // own request, and the gateway doesn't strip them, so "no JWT claim" alone is not enough
        // to trust the header.
    ...
    private async Task ApplyAsync(DbConnection connection, CancellationToken ct)
    {
        ...
        cmd.CommandText = string.IsNullOrEmpty(schema) || schema == "public"
            ? "SET search_path TO public, stores" : $"SET search_path TO \"{schema}\"";
