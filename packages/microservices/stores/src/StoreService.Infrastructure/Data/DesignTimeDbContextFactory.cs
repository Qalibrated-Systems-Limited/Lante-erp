...
/// <summary>Used by EF Core tooling (dotnet ef migrations add / database update). Resolves the
/// connection string exactly like the running app does: appsettings.json ->
/// appsettings.{Environment}.json -> environment variables. Works whether `dotnet ef` runs with
/// cwd at the Api or the Infrastructure project.</summary>
public class DesignTimeDbContextFactory : IDesignTimeDbContextFactory<StoreDbContext>
{
    private const string ApiProject = "StoreService.Api";
    ...
    public StoreDbContext CreateDbContext(string[] args)
    {
        ...
        optionsBuilder.UseNpgsql(connectionString, o => o.MigrationsAssembly("StoreService.Infrastructure"));
