...
/// <summary>
/// Records every fleet write into <see cref="FleetAuditLog"/> — same shape as finance's
...
/// </summary>
public class FleetAuditInterceptor(IHttpContextAccessor httpContextAccessor) : SaveChangesInterceptor
{
    ...
    private void Record(DbContext? db)
    {
        ...
        var entries = db.ChangeTracker.Entries()
            .Where(e => e.Entity is not FleetAuditLog)
            .Where(e => e.State is EntityState.Added or EntityState.Modified or EntityState.Deleted)
            .ToList();

        foreach (var entry in entries)
        {
            db.Add(new FleetAuditLog
            {
