...
/// <summary>
/// Design-time factory for <see cref="TenantFleetServiceDbContext"/>. Emits schema-agnostic
/// migrations that apply into whichever tenant schema the connection's search_path points at.
/// </summary>
public class TenantFleetServiceDbContextDesignTimeFactory : IDesignTimeDbContextFactory<TenantFleetServiceDbContext>
{
    private const string ApiProject = "FleetService.Api";

    public TenantFleetServiceDbContext CreateDbContext(string[] args)
    {
        ...
        var optionsBuilder = new DbContextOptionsBuilder<TenantFleetServiceDbContext>();
        optionsBuilder.UseNpgsql(connectionString, o => o.MigrationsAssembly("FleetService.Infrastructure"));

        return new TenantFleetServiceDbContext(optionsBuilder.Options);
    }
}
