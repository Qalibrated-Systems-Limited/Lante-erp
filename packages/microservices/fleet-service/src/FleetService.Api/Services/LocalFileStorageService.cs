...
public class LocalFileStorageService(IConfiguration config, IHttpContextAccessor httpContextAccessor)
{
    ...
    public IFormFile? ResolveFile(IFormFile? named, string? fieldName = null)
    {
        ...
        // When a specific field name is requested, only match that field — falling back to
        // "any file in the request" would misattribute another action's file (e.g. an odometer
        // photo) to this one whenever the caller only sent one of several possible named files.
    ...
    public async Task<string> SaveAsync(IFormFile file, string folder)
    {
        ...
        if (!AllowedExtensions.Contains(ext))
            throw new InvalidOperationException($"File type '{ext}' is not allowed. Accepted: jpg, jpeg, png, gif, webp.");
