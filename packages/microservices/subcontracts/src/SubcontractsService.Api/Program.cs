...
using System.Text;
using SubcontractsService.Api.Authorization;
using SubcontractsService.Api.Middleware;
using SubcontractsService.Core.Interfaces.Services;
using SubcontractsService.Core.ServiceRegistration;
using SubcontractsService.Infrastructure.Data;
using SubcontractsService.Infrastructure.ServiceRegistration;
...
try
{
    ...
    Log.Information("Starting QaliCore SubcontractsService...");
    ...
    services.AddOpenTelemetry()
        .ConfigureResource(r => r.AddService(serviceName: "subcontracts-service"))
        .WithTracing(t => t
    ...
    // Cross-service HTTP clients — SUB-008 alerts into ticketing's shared Alerts table, and the
    // SUB-005 hard RAMS-approval gate checked against HSE before mobilization is activated.
    services.AddHttpClient("TicketingService");
    services.AddScoped<ITicketingServiceClient, SubcontractsService.Api.Services.TicketingServiceClient>();
    services.AddHttpClient("HseService");
    services.AddScoped<IHseServiceClient, SubcontractsService.Api.Services.HseServiceClient>();
    ...
    services.AddSingleton<SubcontractsAuditInterceptor>();
    services.AddDbContext<SubcontractsDbContext>((sp, options) =>
    {
        options.UseNpgsql(appConnection, sqlOptions =>
        {
            sqlOptions.MigrationsAssembly("SubcontractsService.Infrastructure");
        ...
        options.AddInterceptors(sp.GetRequiredService<SubcontractsAuditInterceptor>());
    });

    // Schema-per-tenant provisioning
    services.AddScoped<SubcontractsService.Infrastructure.Services.ITenantProvisioningService,
                       SubcontractsService.Infrastructure.Services.TenantProvisioningService>();
    ...
    var jwksResolver = new SubcontractsService.Api.Authentication.JwtIssuerSigningKeyResolver(new HttpClient(), jwksUrl);
    ...
    services.AddSwaggerGen(c =>
    {
        ...
        {
            Title = "QaliCore SubcontractsService API",
            Version = "v1",
            Description = "QaliCore — Subcontractor Engagement & Prequalification"
        });
    ...
    app.UseStaticFiles(new Microsoft.AspNetCore.Builder.StaticFileOptions
    {
        ...
        RequestPath = "/uploads/subcontracts"
    });
    ...
    if (app.Environment.IsDevelopment())
    {
        app.UseSwaggerUI(c =>
        {
            c.SwaggerEndpoint("/swagger/v1/swagger.json", "QaliCore SubcontractsService API V1");
            c.RoutePrefix = string.Empty;
            c.DocumentTitle = "QaliCore SubcontractsService API (Dev)";
        });
    }
    else
    {
        app.UseSwaggerUI(c =>
        {
            c.SwaggerEndpoint("v1/swagger.json", "QaliCore SubcontractsService API V1");
            c.RoutePrefix = "swagger";
            c.DocumentTitle = "QaliCore SubcontractsService API";
        });
    }
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        var migrationOptions = new DbContextOptionsBuilder<SubcontractsDbContext>()
        ...
        await using var migrationContext = new SubcontractsDbContext(migrationOptions);
        ...
        var provisioning = scope.ServiceProvider
            .GetRequiredService<SubcontractsService.Infrastructure.Services.ITenantProvisioningService>();
    ...
    Log.Information("QaliCore SubcontractsService started successfully.");
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "QaliCore SubcontractsService terminated unexpectedly.");
    throw;
}
finally
{
