...
namespace SubcontractsService.Infrastructure.Services;
...
/// <summary>
/// Creates and migrates Subcontracts' tables into a tenant's Postgres schema. Copied from
/// HSEService/ComplianceService's TenantProvisioningService.
/// </summary>
...
#pragma warning restore EF1002
            ...
            _logger.LogInformation("Provisioned Subcontracts schema {Schema}", schema);
            return new ProvisioningResult(true, schema, null);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed provisioning Subcontracts schema {Schema}", schema);
            return new ProvisioningResult(false, schema, ex.Message);
        }
    }

    private TenantSubcontractsDbContext BuildTenantContext(string schema)
    {
        ...
        var options = new DbContextOptionsBuilder<TenantSubcontractsDbContext>()
            .UseNpgsql(connectionString, o =>
            {
                o.MigrationsAssembly("SubcontractsService.Infrastructure");
                o.MigrationsHistoryTable("__EFMigrationsHistory", schema);
            })
            .ConfigureWarnings(w => w.Ignore(RelationalEventId.PendingModelChangesWarning))
            .Options;

        return new TenantSubcontractsDbContext(options);
    }
    ...
    private async Task<string?> GrantAppRoleAsync(TenantSubcontractsDbContext ctx, string schema, CancellationToken ct)
    {
        var appRole = _configuration["ProvisioningAppRole"] ?? "qalicore_app";
        if (!SafeSchemaRegex().IsMatch(appRole)) return null;
        try
        {
#pragma warning disable EF1002 // role name is validated by SafeSchemaRegex above — not user input, and identifiers can't be parameterized via ExecuteSqlAsync anyway.
...
#pragma warning disable EF1002 // schema and role names are validated by IsSafeSchema/SafeSchemaRegex above — not user input, and identifiers can't be parameterized via ExecuteSqlAsync anyway.
