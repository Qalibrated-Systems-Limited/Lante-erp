...
/// <summary>Used by `dotnet ef migrations add / database update` for the public-plane SubcontractsDbContext.</summary>
public class DesignTimeDbContextFactory : IDesignTimeDbContextFactory<SubcontractsDbContext>
{
    private const string ApiProject = "SubcontractsService.Api";

    public SubcontractsDbContext CreateDbContext(string[] args)
    {
        ...
        var optionsBuilder = new DbContextOptionsBuilder<SubcontractsDbContext>();
        optionsBuilder.UseNpgsql(connectionString, o => o.MigrationsAssembly("SubcontractsService.Infrastructure"));

        return new SubcontractsDbContext(optionsBuilder.Options);
    }
}
