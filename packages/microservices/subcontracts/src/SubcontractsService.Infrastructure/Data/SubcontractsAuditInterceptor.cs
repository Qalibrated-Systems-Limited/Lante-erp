...
/// <summary>
/// Records every subcontracts write into <see cref="SubcontractsAuditLog"/> — same shape as
/// finance's <c>FinanceAuditInterceptor</c> (#216): intercepting SaveChanges rather than calling a
/// logger at each site means a new entity or a new controller is covered without anyone
/// remembering to add it, and the audit row is written in the same transaction as the change it
/// describes.
/// </summary>
public class SubcontractsAuditInterceptor(IHttpContextAccessor httpContextAccessor) : SaveChangesInterceptor
{
    ...
    private void Record(DbContext? db)
    {
        ...
        var entries = db.ChangeTracker.Entries()
            .Where(e => e.Entity is not SubcontractsAuditLog)
            .Where(e => e.State is EntityState.Added or EntityState.Modified or EntityState.Deleted)
            .ToList();

        foreach (var entry in entries)
        {
            db.Add(new SubcontractsAuditLog
            {
