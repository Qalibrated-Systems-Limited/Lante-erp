...
/// <summary>
/// Design-time factory for <see cref="TenantTicketingDbContext"/>. The model is schema-agnostic, so
/// `dotnet ef migrations add` emits operations with no schema qualifier — they apply into whichever
/// tenant schema the connection's search_path points at.
/// </summary>
public class TenantTicketingDbContextDesignTimeFactory : IDesignTimeDbContextFactory<TenantTicketingDbContext>
{
    private const string ApiProject = "TicketingService.Api";

    public TenantTicketingDbContext CreateDbContext(string[] args)
    {
        ...
        var optionsBuilder = new DbContextOptionsBuilder<TenantTicketingDbContext>();
        optionsBuilder.UseNpgsql(connectionString, o => o.MigrationsAssembly("TicketingService.Infrastructure"));

        return new TenantTicketingDbContext(optionsBuilder.Options);
    }
}
