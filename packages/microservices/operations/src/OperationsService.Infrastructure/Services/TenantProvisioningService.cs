...
/// <summary>Creates and migrates the operations tables into a tenant's Postgres schema.</summary>
...
#pragma warning disable EF1002 // schema name is validated by IsSafeSchema above (line 32) before use — not user input, and identifiers can't be parameterized via ExecuteSqlAsync anyway.
...
#pragma warning restore EF1002
            ...
            _logger.LogInformation("Provisioned operations schema {Schema}", schema);
            return new ProvisioningResult(true, schema, null);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed provisioning operations schema {Schema}", schema);
            return new ProvisioningResult(false, schema, ex.Message);
        }
    }

    private TenantOperationsDbContext BuildTenantContext(string schema)
    {
        ...
        var options = new DbContextOptionsBuilder<TenantOperationsDbContext>()
            .UseNpgsql(connectionString, o =>
            {
                o.MigrationsAssembly("OperationsService.Infrastructure");
            ...
            // (EF8 here — no PendingModelChangesWarning to suppress.)
            .Options;

        return new TenantOperationsDbContext(options);
    }
    ...
    private async Task<string?> GrantAppRoleAsync(TenantOperationsDbContext ctx, string schema, CancellationToken ct)
    {
        var appRole = _configuration["ProvisioningAppRole"] ?? "qalicore_app";
        if (!SafeSchemaRegex().IsMatch(appRole)) return null;
        try
        {
#pragma warning disable EF1002 // role name is validated by SafeSchemaRegex above (line 83) before use — not user input, and identifiers can't be parameterized via ExecuteSqlAsync anyway.
...
#pragma warning disable EF1002 // schema and role names are validated (IsSafeSchema / SafeSchemaRegex, called before this method and above at line 83) before use — not user input, and identifiers can't be parameterized via ExecuteSqlAsync anyway.
